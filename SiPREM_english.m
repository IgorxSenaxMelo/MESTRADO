...
title('Ranking Position Heatmap: Experts vs. S2 Group', ...
...
xtickangle(20);
ylabel('Weight');
title('Fuzzy Criteria Weights (Group FAHP Reference)', 'FontSize', 13, 'FontWeight', 'bold');
grid on;
box on;
hold off;
...
title('Net Flow Comparison between S1 and S2', 'FontSize', 13, 'FontWeight', 'bold');
xlabel('Requirements');
ylabel('\phi_{net}');
legend({'S1 - Institutional FAHP', 'S2 - A Posteriori Aggregation'}, 'Location', 'best');
...
xlabel('\phi_{net} - S1');
ylabel('\phi_{net} - S2');
title('Comparison of Net Flows between S1 and S2', 'FontSize', 13, 'FontWeight', 'bold');
grid on;
box on;
axis equal;
hold off;
...
% title('Ranking Sensitivity: Individual Experts vs. S2 Group', 'FontSize', 14, 'FontWeight', 'bold');
% ylim([0, nReq + 1.5]);
...
title('Requirements with the Largest Ranking Changes between S1 and S2', 'FontSize', 13, 'FontWeight', 'bold');
xlabel('Requirements');
ylabel('|Δ rank|');
grid on;
box on;
...
title('Pearson Correlation of Net Flows (\phi_{net})',...
      'FontSize',13,...
      'FontWeight','bold');
...
title('Comparison of the Most Relevant Requirements in S1 and S2', 'FontSize', 13, 'FontWeight', 'bold');
xlabel('Requirements');
ylabel('\phi_{net}');
...
title('Boxplot of Individual Net Flows by Requirement (S2)', ...
    'FontSize', 13, 'FontWeight', 'bold');
xlabel('Requirements');
ylabel('\phi_{net}');
xtickangle(45);
...
legend({'Aggregated S2 Net Flow'}, 'Location', 'best');
%% =========================================================
...
colorbar('Ticks',[-1 0 1 2], ...
         'TickLabels',{'P- (Outranked)','I (Indifferent)','P (Outranks)','R (Incomparable)'})
...
text(min(U), max(V)*1.06, ...
    sprintf('\\delta = %.2f%% of Information Preserved in the Plane', delta_gaia), ...
...
title('Classification Heatmap by Source',...
      'FontSize',13,'FontWeight','bold');
...
title('Number of Classification Changes per Requirement (Across Experts and Scenarios)', ...
    'FontSize', 13, 'FontWeight', 'bold');
xlabel('Requirements');
ylabel('Number of Class Changes');
grid on;
box on;
%% =========================================================
...
title('Classification Change Matrix between Sources', ...
    'FontSize', 13, 'FontWeight', 'bold');
xlabel('Source');
ylabel('Source');
...
title('Boxplot of Mission Impact Ratings by Requirement', ...
    'FontSize', 13, 'FontWeight', 'bold');
xlabel('Requirements');
ylabel('Impact Rating');
xtickangle(45);
grid on;
box on;
legend({'Mean Impact'}, 'Location', 'best');
...
ylabel('Mean Rating Dispersion (Standard Deviation)');
title('\phi_{net} versus Rating Dispersion by Requirement', ...
    'FontSize', 13, 'FontWeight', 'bold');
grid on;
box on;
hold off;
...
xlabel('\phi_{net} (Aggregated Priority)');
ylabel('Impact Rating Range (max - min)');
title('\phi_{net} versus Mission Impact Disagreement', ...
    'FontSize', 13, 'FontWeight', 'bold');
grid on;
box on;
hold off;
...
for e = 1:nExperts
    ...
    title(sprintf('Deviation of E%d from the Group Median',e),...
          'FontSize',13,...
          'FontWeight','bold');
...
function [idx_km_full, C_sorted, group_names] = get_kmeans_groups(phi_vec, key_idx)
    ...
    group_names(2) = "Group 2";
    group_names(3) = "Group 3";
